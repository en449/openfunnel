...
- [x] **DSGVO GATE — deletion path per data subject, proven across `lead`, `event` and Storage** (§8.7) — the **mechanism is code-complete and tested**, done 2026-08-19 (PHASE-2-PLAN.md §4 WO D3): `find_subject`/`erase_subject` in `supabase/migrations/20260819100000_subject_rights.sql`, 21 scenarios / 67 assertions in `supabase/tests/subject-rights.sql`, green under `scripts/db-test.sh` against a local Postgres with the `check_asserts` tripwire on. Storage turned out to need no walk at all — corrected from the plan's original design (PHASE-2-PLAN.md §4 Decision 2): nothing links a Storage object to a data subject, because ingest stores no uploads (`file` is absent from the console's field types) and every object under `funnel/<slug>/` is the operator's own marketing photography, put there by the console, not lead data. The Löschkonzept is written down as `LOESCHKONZEPT.md` (WO D8). **LIVE 2026-08-27**: `supabase db push` applied this migration, and `find_subject` / `erase_subject` / `subject_matches` are present on the live project (verified with `supabase db query --linked` against `pg_proc`). A deletion request can now actually be executed
- [x] **DSGVO GATE — retention purge job running and logged** (events 90d, leads per-client default 12 months) — the **function is code-complete and tested**, done 2026-08-20 (WO D5): `purge_expired()` in `supabase/migrations/20260819140000_retention_purge.sql` does the three deletions §8.7 promises (events at 90 days, leads at `greatest(client.retention_months, 1)`, a soft delete turned hard 24h later) and logs every run — including one that deleted nothing — into `purge_run`. 42 assertions in `supabase/tests/purge.sql`, red-checked against 13 deliberate breaks. **RUNNING 2026-08-27**: the migration is pushed and `openfunnel-purge` is scheduled at `40 3 * * *` and active on the live project, with the old inline `openfunnel-event-purge` correctly gone — so the two cannot double-count and `purge_run` cannot read a false 0. Verified with `supabase db query --linked` against `cron.job`. One proof run is logged in `purge_run` (2026-08-27 23:45 UTC, all counts 0, `capped` false), which is a pass: nothing was due yet on a project this young. An EMPTY `purge_run` would have been the failure
- [x] **DSGVO GATE — subject search by email/phone across a client's leads** (§8.6) — the console half is done 2026-08-20 (WO D4): `GET /api/admin/subjects` + the Subjects view — pick a client, search, read what is held (soft-deleted, restricted and spam rows shown with their flags, not filtered out, because the data subject is entitled to know a record exists), export as JSON client-side. Verified in a browser against a stubbed PostgREST with two clients (`screenshots/d4-subjects-erase-receipt.png`). **LIVE 2026-08-27**: D3's migration is pushed, so `find_subject` exists and this view returns results for a real client rather than a database error. Not yet exercised against live data — the first real search is the confirmation
